...
export async function checkForUpdates({ packageJsonPath, nodeModulesDir, silent }) {
  try {
    debug("Checking for Claude package updates...");

    const latestVersion = execSync(`npm view ${UPSTREAM_PACKAGE} version`).toString().trim();
