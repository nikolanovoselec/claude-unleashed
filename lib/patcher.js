...
export function applyPatches({ originalCliPath, yoloCliPath }) {
  ...
  const hashPath = yoloCliPath + HASH_CACHE_SUFFIX;
  if (fs.existsSync(yoloCliPath) && fs.existsSync(hashPath)) {
  ...
  const tmpPath = yoloCliPath + '.tmp';
  fs.writeFileSync(tmpPath, patched);
  fs.renameSync(tmpPath, yoloCliPath);

  // Write hash cache
  fs.writeFileSync(hashPath, sourceHash);

  debug(`Patched CLI written to ${yoloCliPath}`);
