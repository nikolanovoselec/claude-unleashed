...
export default new Patch({
  ...
  apply(source) {
    ...
    const unleashedArray = array.map(word =>
      word + UNLEASHED_SUFFIXES[Math.floor(Math.random() * UNLEASHED_SUFFIXES.length)]
    );
    return source.replace(ORIGINAL_ARRAY, JSON.stringify(unleashedArray));
  },
});
